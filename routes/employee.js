...
router.post("/filterEmployee",function(req,res){
	...
	let sql1="SELECT Employee.emp_id,Employee.salary,Employee.name,Employee.joining_date,COUNT(Orders.emp_id) AS cars_sold,persign.emp_id as per,tempSign.emp_id as temp FROM Employee  LEFT JOIN Orders ON Employee.emp_id=Orders.emp_id LEFT JOIN persign ON Employee.emp_id=persign.emp_id LEFT JOIN tempSign ON Employee.emp_id=tempSign.emp_id GROUP BY Employee.emp_id ";
	let entered=false;
          ...
          // console.log("Bhushannnnnn3");
				...
				db.query(sql1,function(err,result1){
					...
					if(err){
						...
						let sql2="SELECT Orders.order_id,Orders.car_id ,Orders.date_order,Orders.dely_date,Orders.cust_id,Orders.extra_id,customer.name FROM Employee,Orders,customer where Employee.emp_id=Orders.emp_id AND customer.cust_id=Orders.cust_id AND Employee.emp_id='"+emp_id+"' order by Orders.dely_date desc LIMIT 3";
						db.query(sql2,function(err,result2){
